...
const elements = {
    ...
    modals: {
        ...
        resetPassword: document.getElementById('resetPasswordModal') // new
    },
    ...
    forms: {
        ...
        resetPassword: document.getElementById('resetPasswordForm') // new
    },
...
function setupEventListeners() {
    ...
    elements.forms.resetPassword.addEventListener('submit', handlePasswordReset); // new
                        ...
                        <div class="comments-list" id="commentsList-${item.id}" style="max-height: 300px; overflow-y: auto; margin-bottom: 1rem;">
                            ${itemComments.length === 0 ? 
                                ...
                                itemComments.map(comment => {
                                    ...
                                    const totalLikes = (comment.likes ? comment.likes.length : 0) + (comment.adminLikeCount || 0);
                                    return `
                                        <div class="comment" style="margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid var(--gray-light);">
                                            ...
                                            <div style="display: flex; align-items: center; gap: 15px; margin-top: 0.5rem;">
                                                ...
                                                ${state.currentUser && state.currentUser.role === 'admin' ? `
                                                    <button onclick="adminEditLike(${comment.id})" style="background: none; border: none; color: var(--gray); cursor: pointer;">
...
// New admin function to edit comment likes
function adminEditLike(commentId) {
    if (!state.currentUser || state.currentUser.role !== 'admin') return;
    const comment = state.comments.find(c => c.id === commentId);
    if (!comment) return;
    const newCount = prompt('Enter new like count (admin only):', comment.adminLikeCount || 0);
    ...
    comment.adminLikeCount = num;
    saveData();
    refreshComments(comment.itemId);
}
...
window.adminEditLike = adminEditLike; // new global function
